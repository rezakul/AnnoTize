...
class AnnotationObject {
   ...
   * @param {TextHighlight} selection the corresponding selection object holding the selected text
  ...
  show(silent=false) {
    ...
    runtime.sidebar.addElement(this.id, this.#annotationBody, silent);
  ...
  */
  ...
  setFragmentTarget(fragment) {
    ...
    // set new selection
    selection = new TextHighlight(range, this.id, fragment, this.styleClass);
    ...
    // inform sidebar about new fragment-target
  ...
  adjustSelection(border, direction) {
    ...
    /*
    let range = getRangeToXPath(this.#target.pathSelector.startPath, this.#target.pathSelector.endPath);
    let s = getXPathToNode(range.startContainer, range.startOffset);
    let e = getXPathToNode(range.endContainer, range.endOffset);
    console.log(s, e);
    */
  }
  ...
  resizeEvent(event) {
    ...
    this.#annotationBody.resizeEvent(event);
  }
}