...
class AnnotationRuntime {
   ...
   */
  constructor(url, active=true, config=null) {
    this.#active = active;
    if (config) {
  ...
  */
  ...
  clickOnPopup() {
    ...
    selected = new SelectionObject(range, uniqueIds.get("annotation"), fragmentTarget, style);
  ...
  closeMenu() {
      this.#settings.closeMenu();
  }

  openTagSetsMenu() {
    this.#settings.openTagSetsMenu();
  }

  /*
  openTagSetMenu(event, tagSetID) {
    let tagSet;
    if (!this.getPlugin("SimpleTagBody").tagSets.has(tagSetID)) {
      console.error('Error: tagSet with id not found', tagSetID);
      return;
    }
    tagSet = this.getPlugin("SimpleTagBody").tagSets.get(tagSetID);
    this.#settings.openTagSetMenu(tagSet);
  }
  */

  openTagSetMenu(event, tagSetID) {
    let tagSet = tagSetPlugin.getTagSetForId(tagSetID);;
    if (!tagSet) {
      return;
    }
    this.#settings.openTagSetMenu(tagSet);
  ...
  openUserMenuCreateNewTagSet(callback) {
    this.#settings.openTagSetCreationMenu(null, callback);
  }

  openUserMenuCreateNewTag(tagSetId, callback) {
    this.#settings.openTagCreationMenu(tagSetId, null, callback);
  }

  openTagMenu(tag) {
    this.#settings.openTagMenu(tag);
  }

  /*
  showTagInformation(event, tagId=null) {
    let tag;
    if (tagId !== null) {
      tag = runtime.getPlugin("SimpleTagBody").getTagForId(tagId);
      if (tag) {
        runtime.openTagMenu(tag);
      }
    } else {
      tag = runtime.getPlugin("SimpleTagBody").getTagForId(event.target.dataset.tagId);
      if (tag) {
        runtime.openTagMenu(tag);
      }
    }
  ...
  */
  showTagInformation(event, tagId=null) {
    let tag;
    if (tagId !== null) {
      tag = tagSetPlugin.getTagForId(tagId);
      if (tag) {
        runtime.openTagMenu(tag);
      }
    } else {
      tag = tagSetPlugin.getTagForId(event.target.dataset.tagId);
      if (tag) {
        runtime.openTagMenu(tag);
      }
    }
  ...
  loadAnnotationFromJSON(annotation, fragment) {
    ...
    selected = new SelectionObject(range, annotation.id, fragmentTarget, style);
  ...
  initializeAnnotationValuesFromJSON(annotation) {
    ...
    annotationObject.annotationBody.initalizeValues(annotation.body);
  ...
  keyupEventTemplateSelection(event) {
    ...
    if (["1", "2", "3", "4", "5"].includes(event.key)) { // escape key maps to keycode `27`
