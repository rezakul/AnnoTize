class AbstractField {
  ...
  initializeValues(val) {
    ...
    if (typeof(val) === "string" || typeof(val) === "number") {
      ...
      console.error("Unsupported type of input value:", val, typeof(val));
      return;
    }
    ...
    for (let i = 0; i < valArray.length; ++i) {
      if (this.#templates.length <= i) {
        ...
        template.initializeValue(valArray[i]);
        this.#templates.push(template);
      } else {
        this.#templates[i].initializeValue(valArray[i]);
      }
    }
  }
