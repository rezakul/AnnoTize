...
class TemplateBody {
   ...
   * @returns {AnnotationSidebarElement} The annotation sidebar object
   */
  get sidebarElement() {
  ...
  initializeValues(values) {
    for (let field of this.#fields) {
      field.initializeValues(values[field.name]);
    }
  }
   ...
   * @param {AnnotationSidebarElement} sidebarElement - the parent sidebar element
   */
  ...
  #initializeConcept(concept) {
    ...
    for (let conceptField of concept.fields) {
      ...
      switch (conceptField.type) {
        case 'textbox':
          fieldGenerator = (abstrField, number, defaultValue) => { return new TextboxField(abstrField, conceptField.name, number, defaultValue, conceptField.maxLength) };
          break;
        case 'text':
          fieldGenerator = (abstrField, number, defaultValue) => { return new TextField(abstrField, conceptField.name, number, defaultValue, conceptField.validation) };
          break;
        case 'number':
          fieldGenerator = (abstrField, number, defaultValue) => { return new NumberField(abstrField, conceptField.name, number, defaultValue, conceptField.min, conceptField.max, conceptField.step) };
          break;
        case 'reference':
          fieldGenerator = (abstrField, number, defaultValue) => { return new ReferenceField(abstrField, conceptField.name, number, conceptField.referenceRole, conceptField.label, conceptField.referencedTypes, conceptField.undirected, defaultValue)};
          break;
        case 'tag':
          fieldGenerator = (abstrField, number, defaultValue) => { return new TagField(abstrField, conceptField.name, number, defaultValue)};
          break;
        default:
